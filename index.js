...
io.on('connection', socket => {
    ...
    socket.on('callAccepted', function (callObject) {
        ...
        new Promise((resolve, reject) => {
            ...
            socket.to(user.socketId).emit('outgoingCallConnected', callObj);
        });
        
        socket.emit('outgoingCallConnected', callObj);
    });
    
    socket.on('hangUp', function (number) {
        new Promise((resolve, reject) => {
            users.getUser(number, resolve);
        }).then((user) => {
            socket.to(user.socketId).emit('hangUp', callObj);
        });
    });
});
